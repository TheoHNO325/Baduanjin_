...
from pPose_nms import getangles,calculate_angle
...
class ImageLoader:
    def __init__(self, im_names, batchSize=1, format='yolo', queueSize=500000):
        self.img_dir = opt.inputpath
        self.imglist = im_names
        self.transform = transforms.Compose([
            transforms.ToTensor(),
...
class VideoLoader:
    def __init__(self, path, batchSize=1, queueSize=500000,target_fps=5):
    ...
    def update(self):
        ...
        fps = stream.get(cv2.CAP_PROP_FPS)  # 获取视频帧率
        target_fps = self.tf # 目标帧率（5帧每秒）
        frame_interval = int(fps / target_fps)  # 计算采样间隔
        
        print("间隔帧数",frame_interval)
        ng = 0
        ...
        for i in range(self.num_batches):
            ...
            while self.Q.full():
                print("Qfull:",self.len()) # 为了解决一个dataloder.len与帧数不一致的疑惑，但是并没有解决
                time.sleep(2)
    ...
    def videoinfo(self):
        ...
        return (self.fourcc,self.tf,self.frameSize)
...
class DetectionLoader:
    def __init__(self, dataloder,det_model, batchSize=1, queueSize=1024000):
        # initialize the file video stream along with the boolean
        # used to indicate if the thread should be stopped or not
...
class DetectionProcessor:
    def __init__(self, detectionLoader, queueSize=1024000):
        # initialize the file video stream along with the boolean
        # used to indicate if the thread should be stopped or not
...
class DataWriter:
    def __init__(self, save_video=False,
                savepath='examples/res/1.avi', fourcc=cv2.VideoWriter_fourcc(*'XVID'), fps=5, frameSize=(640,480),
                queueSize=102400):
        if save_video:
            # initialize the file video stream along with the boolean
            # used to indicate if the thread should be stopped or not
    ...
    def update(self):
        while not self.Q.empty():
            try:
                (boxes, scores, hm_data, pt1, pt2, orig_img, im_name) = self.Q.get(timeout=1)
            except:
                continue
            ...
            if boxes is None:
                if opt.save_img or opt.save_video or opt.vis:
                    ...
                    if opt.save_video:
                        ...
                        for _ in range(5):
                            self.stream.write(img)#save the video
            else:
                with torch.no_grad():
                # location prediction (n, kp, 2) | score prediction (n, kp, 1)
                    if opt.matching:
                        ...
                        # preds[n][k] = [x,y,val],第n个样本的第k个keypoint的坐标+value
    ...
    def save(self, boxes, scores, hm_data, pt1, pt2, orig_img, im_name):
        ...
        try:
            self.Q.put((boxes, scores, hm_data, pt1, pt2, orig_img, im_name),timeout=1)
        except:
            print("Warning:Save out of time")
...
class pthWriter(DataWriter):
    def __init__(self,save_video=False,
                savepath='examples/res/1.avi', fourcc=cv2.VideoWriter_fourcc(*'XVID'), fps=5, frameSize=(640,480),
                queueSize=102400,start_frame=5):
        DataWriter.__init__(self,save_video=False,
                    savepath='examples/res/1.avi', fourcc=cv2.VideoWriter_fourcc(*'XVID'), fps=5, frameSize=(640,480),
                    queueSize=102400)
        self.final_result = torch.zeros(200,42)
        self.start = start_frame

    def update(self):
        while not self.Q.empty() and self.start < 205:
            try:
                (boxes, scores, hm_data, pt1, pt2, orig_img, im_name) = self.Q.get(timeout=1)
            except:
                continue
            ...
            if boxes is None:
                ...
                # location prediction (n, kp, 2) | score prediction (n, kp, 1)
                    ...
                    keylist = result[0]['keypoints'].flatten()
                    _,angle_list = getangles(keylist)
                    angle_list = torch.tensor(angle_list)
                    result = torch.cat((keylist,angle_list))
                    assert len(result)==self.final_result.shape[1],"Mismatched shape for cat(keylist,anglelist)"
                    self.final_result[self.start-5] = result
