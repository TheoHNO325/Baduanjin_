...
def write_json(all_results, outputpath, video_name,for_eval=False):
    '''
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            # result['angle_keys'] = {'left_arm_bend':(10, 8, 6),
            #                         'right_arm_bend':(9, 7, 5),
            #                         'left_armpit':(8, 6, 12),
            #                         'right_armpit':(7, 5, 11),
            #                         'left_waist_leg':(6, 12, 14),
            #                         'right_waist_leg':(5, 11, 13),
            #                         'left_knee':(12, 14, 16),
            #                         'right_knee':(11, 13, 15)}
            result['angle_keys'] = getangles(keypoints)
    ...
    if form == 'cmu': # the form of CMU-Pose
        ...
        with open(os.path.join(outputpath,video_name+'.json'), 'w') as json_file:
            json_file.write(json.dumps(json_results))

    return json_results

def write_pth(all_results, outputpath, video_name,for_eval=False):
    form = opt.format
    path = os.path.join(outputpath,video_name+'.pth')
